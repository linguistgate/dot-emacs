...
Set to nil; no need to confirm babel evaluations of code.
#+BEGIN_QUOTE
...
Org modules seem to be a bit like an internal package system. There are a number of Org packages that are set under =org-modules=.
