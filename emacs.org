...
For some reason, =require '(bind-key)= needs to be overtly stated (and seemingly outside of =eval-when-compile= for =:bind= personal keybindings to work with byte compilation (MEA - [2018-11-03 Sat])
...
#+BEGIN_SRC emacs-lisp
  (use-package magit
    :preface (message "I'm here at byte-comile time.")
...
Org modules seem to be a bit like an internal extension system, with some modules comprising Org's core and others external to it. There are a number of Org extensions that are initialized as =org-modules=.
- The package =org-protocol= allows for capturing webpages and links while browsing and importing that into predefined capture templates.
- Describe the rest of these if I use them; otherwise, get rid of them.

Org-modules' individual behavior is established under their own headings below.
...
I keep the number of /agenda/ files small, currently.
...
#+BEGIN_SRC emacs-lisp
  (use-package org-agenda
    ...
    (org-agenda-files '("~/org/organizer.org"
                        "~/org/teaching/teaching-admin-tasks.org")))
...
#+BEGIN_SRC emacs-lisp
  (setq org-refile-targets '(("~/org/organizer.org" :maxlevel . 5)
                             ...
                             ("~/org/freelance.org" :maxlevel . 5)
                             ("~/org/reading.org" :maxlevel . 5)
                             ("~/org/teaching.org" :maxlevel . 5)
...
One reason I use Emacs and Org-mode is that it allows me to "build a second brain" and to seamlessly integrate writing, reading, knowledge management, and workflows from many areas of my life. I don't think that Emacs can solve all workflow and management issues, but Org-mode is an enticing and highly (addictive?) method of using plain text to organize my world(s).

My refile targets are associated with the principle of using Emacs and Org, in particular, as a way to build a second brain ("B.A.S.B.").

These ideas derive from the knowledge management principles developed by Tiago Forte. Two excellent resources I've used in developing my own thinking are the following posts from Michael Fogleman ([[@tasshingfoleman][@tasshinfogleman]]):

- [[https://praxis.fortelabs.co/building-a-second-brain-in-emacs-and-org-mode-faa20ae06fc/][Building a Second Brain in Emacs and Org-Mode]]
- [[https://medium.com/@tasshin/implementing-a-second-brain-in-emacs-and-org-mode-ef0e44fb7ca5][Implementing a Second Brain in Emacs]]

Building a Second Brain /extends/ GTD principles by adding three additional strategies:

1. Getting Things Done (GTD) :: Capture, track, and opportunistically surface appropriate tasks across many projects
