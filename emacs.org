...
I am currently using =org-journal= as a way to capture daily tasks that don’t fall neatly into project cycles. These should be tasks that can be marked with a GTD tag (e.g., =@home=, =@errands=, etc.).
...
The =:custom= block establishes the location of the agenda and enforces =org-agenda= integration.

/Changing the output file format has resulted in a number of bugs (i.e., non-carryover, non-automatic invocation of =Journal= mode, etc./
