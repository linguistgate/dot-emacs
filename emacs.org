...
- [[#configuration][Configuration]]
  ...
  - [[#emacs-as-web-server][Emacs as Web Server]]
...
This structure represents about a year of thinking through task, workflow, and knowledge management systems. I don’t think it’s my final word, but it does reflect best where I want to lay my energies. In large measure it incorporates aspects of bullet journaling (via daybook), 
...
In setting up the =org-agenda package, *I’ve found it necessary to set =enable-local-variables= to nil* and simply run them on an as-needed basis. The reason is that when I run =emacs --daemon=, I get requests to run the variables. However, entering any value (=y=, =n=, =!=, etc.) simply causes the daemon process to hang.
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (setq org-agenda-custom-commands
        '(("g" . "Areas")
          ...
          ("G" "Areas Block Agenda"
           ((tags-todo "@digital")
            (tags-todo "@email")
            (tags-todo "@financial")
            (tags-todo "@reading"))
           nil                                              ;; i.e., no local settings
           ("~/publishing/private_html/next-actions.html")) ;; exports block to this file with C-c a e
          ("D" "Daybook Block Agenda"
           ((agenda "" ((org-agenda-span 1)
                        (org-agenda-entry-types '(:timestamp :sexp :scheduled :deadline :journal))))               ;; limits the agenda display to a single day
            (tags-todo "+PRIORITY=\"A\"")
            (tags-todo "@email|@phone|@message")
...
*** The Super Agenda
**** COMMENT Currently out

The package [[https://github.com/alphapapa/org-super-agenda][org-super-agenda]] provides nicer formatting and lots of opportunities for ordering by context, tag, and so on. To keep it persistent, it needs to be set with =setq=.
...
#+BEGIN_SRC emacs-lisp
  (use-package org-journal
    ...
    (org-journal-file-type 'daily)                                                                 ; stores in daily files
...
[2019-04-14 Sun]
I’ve moved out of the =Areas= model. There is now a “hub” (=~/org/hub=) with a central =systems.org= file that encapsulates Areas-esque needs (inventories, habits, recurring household chores, financials). Many parts of it are encrypted. The Commonplace Book idea is in full development now; I have a dedicated (=~/org/cpb=) directory that contains files to be used for notebooks, quotes, vocabulary, and digital scrapbooking.

I’ve made a general move away from using =~/org= as the root node for everything. I’ve gone back to using the Linux home directory as the logical place to store relevant subsets of projects and resources that are checked out from my home server. The =~/org= directory proper handles agenda, capture, scrapbooking, journaling, and daybooking.
...
#+BEGIN_SRC emacs-lisp
  (use-package org-ref
    ...
    :config
    (setq org-ref-notes-directory "~/org/cpb/"
          org-ref-pdf-directory "~/resources/articles/"
          org-ref-bibliography-notes "~/org/cpb/readding.org"
          org-ref-default-bibliography '("~/resources/bibliographies/mainref.bib")
          ))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-static-blog-publish-directory "~/publishing/phantisocracy/")
  (setq org-static-blog-posts-directory "~/publishing/phantisocracy/posts/")
  (setq org-static-blog-drafts-directory "~/publishing/phantisocracy/drafts/")
...
I’m not super sure I’ll want this package around forever. *It’s not on MELPA* [2019-04-14 Sun] but does a decent job giving me “smart quotes” à la WYSIWYG processing, similar to how quotes are entered manually in LaTeX.
